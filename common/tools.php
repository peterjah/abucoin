...
function async_arbitrage($symbol, $sell_market, $sell_price, $buy_market, $buy_price, $size, $arbId)
{
  ...
  if ($pid == -1) {
     ...
     print_dbg("SOLD {$status['sell']['filled_size']} $alt on {$sell_api->name} at {$status['sell']['price']}. expected {$sell_product->book['bids']['price']} orderprice: {$sell_product->book['bids']['order_price']}\n",true);
  } else {
     // we are the child
     print "I am the child pid = $pid\n";
     $status['buy'] = place_order($buy_market, 'limit', $symbol, 'buy', $buy_price, $size, $arbId);
     print_dbg("BOUGHT {$status['buy']['filled_size']} $alt on {$buy_api->name} at {$status['buy']['price']}. expected {$buy_product->book['asks']['price']} orderprice: {$buy_product->book['asks']['order_price']}\n",true);
     exit();
  }
